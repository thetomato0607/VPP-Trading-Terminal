...
These tests make LIVE, unmocked network calls to api.octopus.energy.
...
LIVE_API_SKIP = pytest.mark.skipif(
    not os.environ.get("RUN_LIVE_API_TESTS"),
    reason="requires live network access to api.octopus.energy",
)
...
def test_3_fetch_24h_forecast():
    ...
    assert len(results) >= 48, (  # 48 half-hours = 24 hours
        f"Only {len(results)} prices available (need 48 for 24h)"
    )
...
def test_4_all_regions():
    ...
    assert len(available) > 0, "No regions have Agile tariff available"


def main():
    """Run all tests as a standalone script (bypasses the pytest skip marker)."""
    print("\n" + "="*70)
    ...
    # Run tests, converting assertion/other errors into pass/fail results
    results = {}
